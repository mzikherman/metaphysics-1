...
export default async () => {
  // Rule: Breaking v2 changes generally require a Reaction update.
  const downloadMetaphysicsSchema = async () => {
    const response = await fetch(
      "https://raw.githubusercontent.com/artsy/metaphysics/master/_schemaV2.graphql"
    )
  ...
  if (messages.length) {
    warn(
      `The V2 schema in this PR is incompatible with production. Remember to update Reaction if necessary:\n\n${messages}`
    )
  }
